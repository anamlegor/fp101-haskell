...
doubleSecond ds = map (\(x,y) -> if even y then (2 * x) else x) (zip ds [1..length ds])
...
sumDigits [] = 0
sumDigits (x:xs) = (sumX x) + (sumDigits xs)
  where sumX x = (if x > 9 then sumDigits (toDigitsRev x) else x)   
